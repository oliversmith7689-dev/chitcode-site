...
const LaunchPlan: React.FC = () => (
  <div className="container-x">
    <div className="card p-6 sm:p-10 md:p-14">
      <h2 className="text-3xl sm:text-4xl md:text-5xl">План запуска</h2>
      <ol className="mt-8 md:mt-10 rounded-inner md:rounded-card bg-brand-purple p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-4">
        {STEPS.map((s, i) => {
          const acid = i % 2 === 1;
          return (
            <li key={s.title} className={`rounded-inner px-5 py-5 sm:px-7 sm:py-6 flex gap-5 sm:gap-8 items-start ${acid ? 'bg-brand-acid text-brand-ink' : 'bg-white text-brand-ink'}`}>
